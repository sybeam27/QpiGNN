...
# result folder & file
root_dir = f"./gqnn_results"
csv_dir = os.path.join(root_dir, 'eval')
os.makedirs(csv_dir, exist_ok=True)
pdf_dir = os.path.join(root_dir, 'img')
os.makedirs(pdf_dir, exist_ok=True)

eval_csv_file = os.path.join(csv_dir, f'{args.model}_eval_{args.dataset_name}.csv')
...
if args.model == 'SQR':
    ...
    train_eval = {'Eval': 'train','Model': args.model, **train_eval}
    train_new_row = pd.DataFrame([train_eval])

    if os.path.exists(eval_csv_file):
        df_tr = pd.read_csv(eval_csv_file)
        df_tr = pd.concat([df_tr, train_new_row], ignore_index=True)
    else:
        df_tr = train_new_row
    df_tr.to_csv(eval_csv_file, index=False, float_format='%.3f')
    ...
    test_eval = {'Eval': 'test', 'Model': args.model, **test_eval}
    test_new_row = pd.DataFrame([test_eval])

    if os.path.exists(eval_csv_file):
        df_ts = pd.read_csv(eval_csv_file)
        df_ts = pd.concat([df_ts, test_new_row], ignore_index=True)
    else:
        df_ts = test_new_row
    df_ts.to_csv(eval_csv_file, index=False, float_format='%.3f')
    ...
    train_eval = evaluate_model_performance(train_low_preds, train_upper_preds, train_targets, target=args.target_coverage)
    train_eval = {'Eval': 'train', 'Model': args.model, **train_eval}
    train_eval['lambda_factor'] = args.lambda_factor
    train_new_row = pd.DataFrame([train_eval])
    ...
    test_eval = evaluate_model_performance(test_low_preds, test_upper_preds, test_targets, target=args.target_coverage)
    test_eval = {'Eval': 'test', 'Model': args.model, **test_eval}
    test_eval['lambda_factor'] = args.lambda_factor
    test_new_row = pd.DataFrame([test_eval])
    ...
    train_targets = train_data.y.cpu().numpy()
    train_eval = evaluate_model_performance(train_low_preds, train_upper_preds, train_targets, target=args.target_coverage)
    train_eval = {'Eval': 'test', 'Model': args.model, **train_eval}
    train_new_row = pd.DataFrame([train_eval])
